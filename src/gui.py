...
class BookBuilderGUI:
    ...
    def render_sidebar(self):
        ...
        self.config['instructor_model'] = st.sidebar.selectbox(
            "Instructor Agent Model", model_options, index=0
        )
        self.config['brainstormer_idea_model'] = st.sidebar.selectbox(
            "Brainstormer (Idea) Model", model_options, index=0
        )
        self.config['brainstormer_critique_model'] = st.sidebar.selectbox(
            "Brainstormer (Critique) Model", model_options, index=0
        )
        self.config['writer_model'] = st.sidebar.selectbox(
            "Writer Model", model_options, index=0
        )
        self.config['writing_reviewer_model'] = st.sidebar.selectbox(
            "Writing Reviewer Model", model_options, index=0
        )
        self.config['translator_model'] = st.sidebar.selectbox(
            "Translator Model", model_options, index=0
        )
            ...
            for event in app.stream(state, graph_config):
                for node_name, node_state in event.items():
                    if node_name == "instructor":
                        status_text.text("👨‍🏫 Instructor Agent is gathering requirements...")
                        progress_bar.progress(10)

                    elif node_name == "brainstorming_idea_writer":
                        status_text.text("💡 Brainstorming Agent is creating story outline...")
                        progress_bar.progress(30)

                    elif node_name == "writer":
                        chapter_num = node_state.get('current_chapter', 0) + 1
                        status_text.text(f"✍️ Writer Agent is writing Chapter {chapter_num}...")
                        progress_bar.progress(50 + (chapter_num / self.config['n_chapters']) * 30)

                    elif node_name == "assembler":
                        status_text.text("📚 Assembling final book...")
                        progress_bar.progress(95)

            # Update session state with results
            if 'content' in node_state:
                st.session_state.book_content = node_state['content']
                st.session_state.book_title = node_state.get('book_title', 'Untitled Book')
                st.session_state.book_prologue = node_state.get('book_prologue', '')
    ...
    def render_main_content(self):
        ...
        st.title("📚 Book Builder AI with OpenRouter")
    ...
    def render_setup(self):
        ...
        You can now use **OpenRouter** models along with other providers!

        ### Features:
        - 🤖 Multiple AI model providers (OpenAI, Google, Meta, Amazon, DeepSeek, **OpenRouter**)
